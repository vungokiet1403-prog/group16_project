...
exports.login = async (req,res)=>{
  ...
  if(!u || !(await u.checkPassword(password))) return res.status(400).json({message:"Invalid credentials"});
...
exports.forgotPassword = async (req,res)=>{
  ...
  u.resetPasswordExpires = new Date(Date.now()+1000*60*30); // 30m
  await u.save();
  // Demo: trả token ra để test (thay vì gửi email)
  res.json({message:"Reset token generated", token});
};

exports.resetPassword = async (req,res)=>{
  const {token, newPassword} = req.body;
  const u = await User.findOne({resetPasswordToken: token, resetPasswordExpires: {$gt: new Date()}});
